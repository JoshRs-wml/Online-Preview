...
- **极致轻量自研组件**：
  ...
  - `CustomPdfViewer`：基于 `pdfjs-dist` 核心 API 构建的纯 Canvas 逐页渲染引擎。彻底抛弃原生 Iframe 方案，完美支持自定义缩放/旋转形变，并从根本上杜绝了浏览器的原生下载与打印行为。
  - `CustomVideoViewer`：沉浸式防下载（`nodownload`）HTML5 视频播放器，限制最大高度防撑爆。
- **双屏附件对比 (Dual-pane Comparison)**：为严谨的业务审核场景设计了左右分屏对比模式。支持两份附件的独立操作（如分别放大细节），也支持开启“联动模式 (Sync Mode)”，实现缩放、旋转的完美同步。
- **批量预览工作台**：自带“左侧队列 + 右侧预览”经典布局，支持批量传入文件列表，通过快捷按钮无缝“上一份/下一份”切换。
- **丰富的操作矩阵**：内置了放大、缩小、还原比例、顺时针/逆时针旋转、水平/垂直翻转、全屏、一键下载功能，且所有形变均基于 CSS Transform 高性能实现。
- **工业级稳定性（4 重防御）**：
...
| **PDF** | `.pdf` | `pdfjs-dist` Canvas 逐页绘制 (替代 Iframe，支持形变与防下载) |
